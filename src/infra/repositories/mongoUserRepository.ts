...
export class MongoUserRepository implements ILoadUserByEmail, ISave {
    ...
    async loadUserByEmail(email: string): Promise<Pick<IUserModel, "email" | "_id" | "password">> {
        const user : Pick<IUserModel, "email" | "_id" | "password"> = await this.entity.findOne({email}, {'username' : 0 , 'updatedAt' : 0, 'createdAt' : 0,'role' : 0})
        return user
    }
